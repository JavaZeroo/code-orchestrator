...
# 独立于 co-server 运行(auto-merge 应 detached 地 spawn 它，使其在 co-server 重启期间存活)。
# 流程:记回滚点 → pull → 拒绝含迁移的自动部署(迁移是护栏) → 预检(install+typecheck+test)
#       → rebuild web → 重启 co-server → 探活;不健康则自动 git reset 回滚 + 重启老代码。
# 退出码:0=成功 1=预检未过已撤销(零downtime) 2=部署后不健康已回滚 3=回滚也失败(需人工) 4=前置错误
set -uo pipefail
...
# 预检失败:co-server 还没重启、仍跑老代码(内存)——只复原树，零 downtime
abort() {
  log "✋ 预检未过，撤销(co-server 未动，仍跑老代码)"
  git reset --hard "$BEFORE" --quiet
  exit 1
}
# 部署后不健康:co-server 已重启到坏代码——git 回滚 + rebuild + 重启老代码 + 探活
rollback() {
  log "⏪ 回滚 → $BEFORE"
  git reset --hard "$BEFORE" --quiet
  pnpm --filter @co/web build >/dev/null 2>&1
  pm2 restart co-server >/dev/null 2>&1
  if wait_health 15; then log "已回滚，co-server 健康"; exit 2; fi
...
# 预检:装依赖 + typecheck + test(任一挂即 abort，co-server 不动)
log "预检: install"; pnpm install --frozen-lockfile --prefer-offline >/dev/null 2>&1 || { log "install 失败"; abort; }
log "预检: typecheck"; pnpm -r typecheck >/dev/null 2>&1 || { log "❌ typecheck 失败 — 不部署"; abort; }
log "预检: test"; pnpm -r test >/dev/null 2>&1 || { log "❌ test 失败 — 不部署"; abort; }
