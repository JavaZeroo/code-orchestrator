...
- 引擎支持四种节点：
  * type="agent"：id, title, model, prompt, permissionMode?(自主执行节点用 "bypassPermissions"), effort?, reviseLoop?{target,maxRounds}, cwd?, machine?{labels[]}, outputs?
  ...
  * type="meeting"：id, title, participants:[{model,role?},…]≥2, rounds?, arbiter?({model}|"vote"|"human")——多模型独立评审+交叉反驳
